...
func TestSchema(t *testing.T) {
	schemaBackup := "../pkg/yq/schema/schema.json.bak"
	schemaPath := "../pkg/yq/schema/schema.json"
	...
	output := &bytes.Buffer{}
	cmd := exec.Command("../bin/mixins/yq/yq", "schema")
	cmd.Stdout = output
	cmd.Stderr = output

	err = cmd.Start()
	require.NoError(t, err, "failed to start the yq schema command")

	err = cmd.Wait()
	t.Log(output)
	require.NoError(t, err, "yq schema failed")
}
